#!/bin/bash
#
# Initialize Configs Backup Repository
# Run this once to set up the restic repository in S3
#
# Reuses the same encryption key as the Foundry and Logs backups (per plan)
#
# Prerequisites:
...
echo "=========================================="
echo "Configs Backup Repository Setup"
echo "=========================================="
echo ""
...
if [[ ! -f "$BACKUPS_DIR/secrets/restic.key" ]]; then
    ...
    echo "Reuse the same key as the Foundry/Logs backups (per the backup plan)"
    exit 1
fi

echo "✓ restic installed: $(restic version | head -1)"
echo "✓ AWS credentials found"
echo "✓ Restic password found (shared with Foundry and Logs backups)"
echo ""
...
echo "Encryption: Client-side via restic (same key as Foundry and Logs)"
echo "Retention:  ${CONFIGS_RETENTION_DAILY} daily + monthly forever"
echo ""
...
echo "     $SCRIPT_DIR/configs-backup.sh"
echo ""
...
echo "     sudo systemctl start configs-backup.timer"
echo "=========================================="
